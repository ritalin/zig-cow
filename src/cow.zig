...
pub fn Cow(comptime sentinel_val: ?comptime_int) type {
    return struct {
        ...
        inline fn initInstanceInternal(allocator: std.mem.Allocator, value: Slice, mode: CloneMode) *InternalPresentation {
            ...
            const buf = buf: {
                if (comptime sentinel_val) |v| {
                    if (v == 0) {
                        break :buf if (mode == .edit) allocator.dupeZ(u8, value) catch @panic("OOM") else value;
                    }
                    else {
                        @compileError(std.fmt.comptimePrint("Unsupported sentinel: {}", .{v}));
                    }
                }
                else {
                    break :buf if (mode == .edit) allocator.dupe(u8, value) catch @panic("OOM") else value;
                }
            };
...
const CStringCow = Cow(0);

test "Modify instance from centinel literal" {
    const allocator = std.testing.allocator;

    const hw: [:0]const u8 = "Hello World";
    var s = CStringCow.initAsCopy(allocator, hw);
    defer s.deinit();
