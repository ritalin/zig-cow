...
pub const StringCow = Cow;

const Slice = []const u8;
const WritableSlice = []u8;
...
pub const Cow = struct {
    ...
    pub fn initAsCopy(allocator: std.mem.Allocator, value: Slice) Cow {
        return .{
    ...
    pub fn initAsMove(allocator: std.mem.Allocator, value: Slice) Cow {
        return .{
    ...
    pub fn clone(self: *Cow, allocator: std.mem.Allocator) Cow {
        self.ptr.share();
        return .{
    ...
    pub fn assignAsMove(self: *Cow, other: *Cow) void {
        var old_ptr = self.ptr;
        defer old_ptr.unshare();
...
test "Mutate instance from literal" {
    ...
    s1.mut( 
        struct {
            pub fn newBuf(a: std.mem.Allocator, old: Readable) ![]const u8 {
